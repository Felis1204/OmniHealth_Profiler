...
///
/// POSIX (Linux/macOS):          调用 gmtime_r(&t, &tm)    (参数顺序: time_t*, tm*)
/// MSVC/MinGW:                   调用 gmtime_s(out, in)    (参数顺序: tm*, time_t*)
///
/// @param t  输入时间 (epoch)
/// @param out 输出 UTC tm 结构
inline void gmtimeCompat(const std::time_t* t, std::tm* out) {
#if defined(HM_PLATFORM_APPLE) || defined(HM_PLATFORM_LINUX)
    gmtime_r(t, out);
#else
    // MSVC 和 MinGW 均提供 gmtime_s
    gmtime_s(out, t);
#endif
}
...
///
/// POSIX (Linux/macOS):          调用 timegm(tm)           (GNU/BSD 扩展)
/// MSVC/MinGW:                   调用 _mkgmtime(tm)         (VS2015+ / MinGW-w64)
///
